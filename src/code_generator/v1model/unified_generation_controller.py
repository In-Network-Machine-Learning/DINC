...
def generator(Allocation, P4_Information, Network_Topology, manually_config = True):
    ...
    for node_idx in Allocation.keys():
        ...
        if DINC_config['p4 config']['begin test'] == "T":
            file_name = 's' + str(node_idx) + '.p4'
            p4_file = DINC_config['directory config']['work'] + '/src/test/' + DINC_config['test config']['targets'] + '/test_environment/P4/' + file_name
        else:
...
def test_system(Allocation, P4_Information, Network_Topology, manually_config = True):
    DINC_config = reload_DINC_config('src/configs/DINC_config.json')
    generator_path = DINC_config['directory config']['work'] + '/src/code_generator/' + DINC_config['p4 config'][
        'generator']
