...
def device_list_to_path_reality(G, adjacent_matrix, in_device_list, out_device_list, DINC_config):
    ...
    for i in in_device_list:
        for o in out_device_list:
            ...
            if i == o:
                continue
            for path in nx.all_simple_paths(G, source=i, target=o,cutoff=5):
                if len(path)==5:
                    if list(set(range(DINC_config['network config']['n_core'])).intersection(path)):
                        ...
                        if path[0]>=DINC_config['network config']['n_core'] and path[-1:][0]>=DINC_config['network config']['n_core']:
                            # print('selected')
                            P[path_idx] = path
                            path_idx += 1
                    # else:
