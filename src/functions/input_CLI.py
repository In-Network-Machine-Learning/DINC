...
def CLI_loop(question, default, check_dir_existance, check_available_options, option_address):
    while True:
        ...
        if check_dir_existance:
            if not check_available_options:
                ...
                input_dir = os.getcwd() + option_address + '/' + CLI_input
...
def take_CLI_input(DINC_config, config_type, config_name, question, default, manully_input, check_dir_existance = False, check_available_options = False, option_address = '', numeric = False):
    if not manully_input:
        try:
            ...
            CLI_input = CLI_loop(question, default, check_dir_existance, check_available_options, option_address)
    else:
        try:
            _ = DINC_config[config_type]
        except Exception as e:
